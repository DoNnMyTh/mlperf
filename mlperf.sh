...
emit_auto_config() {
    ...
    # MAX_STEPS cap — upstream config_common_*.sh defaults ~1.2M steps
    # (~28 days on 4×H200). Preview runs cap at 100, full defers to
    # upstream (compliance run).
    local max_steps val_check
    if [[ "$run_mode" == "preview" ]]; then
        max_steps=100; val_check=50
    else
        max_steps=""; val_check=""   # let upstream config_common_* decide
    fi
    cat > "$out" <<EOF
...
case "$topo" in
    "single-node"*|"multi-node"*)
        # Run-length toggle. Preview caps MAX_STEPS=100 (~mins). Full
        # defers to upstream (~days-weeks) — only useful for compliance.
        rmode_opts=("preview (MAX_STEPS=100 — minutes to hours)" "full (upstream default ~1.2M steps — days)")
        rmsel=$(pick "Run length" "${rmode_opts[@]}")
        run_mode=$( (( rmsel == 1 )) && echo preview || echo full )
        if [[ "$topo" == "single-node"* ]]; then
            nnodes=1; ngpu_per_node="$AUTO_NGPU"
        else
        ...
        CFG_FILE="$AUTO_CFG"
        info "Generated: $(basename "$AUTO_CFG")  (${nnodes}×${ngpu_per_node} GPUs, $run_mode)"
        ;;
    "advanced"*)
