...
emit_auto_config() {
    ...
    # Precision from arch capability. Env override lets post-preview menu
    # force FP8 on/off for throughput probes.
    ...
    [[ -n "${MLPERF_FP8:-}" ]]        && fp8="$MLPERF_FP8"
    [[ -n "${MLPERF_FP8_HYBRID:-}" ]] && fp8_hybrid="$MLPERF_FP8_HYBRID"
    ...
    # MAX_STEPS: preview caps fast; full defers to upstream compliance
    # steps. Env overrides let the post-preview menu request short
    # convergence / throughput probe / custom step counts without
    # re-editing this function.
    ...
    [[ -n "${MLPERF_MAX_STEPS:-}" ]]          && max_steps="$MLPERF_MAX_STEPS"
    [[ -n "${MLPERF_VAL_CHECK_INTERVAL:-}" ]] && val_check="$MLPERF_VAL_CHECK_INTERVAL"
    [[ -n "${MLPERF_WARMUP_STEPS:-}" ]]       && warmup="$MLPERF_WARMUP_STEPS"
    cat > "$out" <<EOF
...
if (( ec == 0 )); then
    ...
    case "$METHOD" in
        ...
        docker|bare|bare_multi|smoke|smoke_bare)
            ...
            # Preview just proved the topology boots + trains. Offer a
            # menu of next-run options with ETAs derived from the cached
            # step-time. Re-execs with MLPERF_AUTO_YES=1 so saved answers
            # replay silently; any run_mode / MAX_STEPS override forwards
            # through env and overrides AUTO config emission.
            if [[ "${run_mode:-}" == "preview" ]] && (( IS_CUSTOM == 0 )); then
                ...
                _pv_stp=$(_step_time_lookup "$WL_NAME" "${AUTO_GPU_ARCH:-0}" "${NGPU:-1}")
                _pv_per="${_pv_stp%%$'\t'*}"
                _pv_src="${_pv_stp#*$'\t'}"
                [[ -n "$_pv_per" ]] && info "Step-time ($_pv_src): ${_pv_per}s/step"
                _eta(){ # $1=steps → formatted ETA using measured step-time + eval overhead
                    local s="$1" eval_s="${_EVAL_TIME_TABLE[$WL_NAME]:-60}" vci="${VAL_CHECK_INTERVAL:-1000}"
                    [[ -z "$_pv_per" ]] && { echo "?"; return; }
                    local train=$(awk -v s="$s" -v p="$_pv_per" 'BEGIN{printf "%.0f", s*p}')
                    local n=0; (( vci > 0 )) && n=$(( s / vci ))
                    local tot=$(( train + n * eval_s ))
                    _fmt_duration "$tot"
                }
                echo
                _opts=(
                    "Full MLPerf compliance run (MAX_STEPS from upstream; ETA $(_eta "${MAX_STEPS_UPSTREAM:-12000}"))"
                    "Short convergence trace (MAX_STEPS=1000, watch loss descend; ETA $(_eta 1000))"
                    "FP8 throughput probe   (MAX_STEPS=50, FP8 forced; ETA $(_eta 50))"
                    "Custom MAX_STEPS"
                    "Stop here"
                )
                _sel=$(pick "Next run" "${_opts[@]}")
                case "$_sel" in
                    1) export MLPERF_RUN_MODE=full   MLPERF_AUTO_YES=1 ;;
                    2) export MLPERF_RUN_MODE=preview MLPERF_AUTO_YES=1 MLPERF_MAX_STEPS=1000 MLPERF_VAL_CHECK_INTERVAL=200 ;;
                    3) export MLPERF_RUN_MODE=preview MLPERF_AUTO_YES=1 MLPERF_MAX_STEPS=50  MLPERF_FP8=True MLPERF_FP8_HYBRID=True ;;
                    4) _steps="$(ask 'MAX_STEPS' 500)"
                       export MLPERF_RUN_MODE=preview MLPERF_AUTO_YES=1 MLPERF_MAX_STEPS="$_steps" ;;
                    5) info "To launch full later: MLPERF_RUN_MODE=full bash mlperf.sh" ;;
                esac
                if (( _sel >= 1 && _sel <= 4 )); then
                    info "Re-executing: MLPERF_RUN_MODE=$MLPERF_RUN_MODE ${MLPERF_MAX_STEPS:+MAX_STEPS=$MLPERF_MAX_STEPS} AUTO_YES=1"
